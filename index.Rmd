...
**The graph shows the relationship between the Social Progress Index and per capita CO₂ emissions.**
...
### **Gross Domestic Product per capita and CO₂ emissions per capita**
...
g2<-ggplot(sd,
  ...
  geom_point(aes(size= population, ids = country, color = co2_percap, text = sprintf(
    ...
    alpha = .7) +
  geom_hline(yintercept= 1.9 , linetype="dashed", color = "darkgrey", size= 1) +
  ylab("CO2 emissions (logarithmic scale)") +
...
**The graph shows the relationship between per capita GDP and per capita CO₂ emissions.**
...
**The graph shows the relationship between SPI tier 1 countries (highest SPI scores) and per CO₂ emissions.**
...
**The graph shows the relationship between SPI tier 6 countries (with the lowest SPI scores) and per CO₂ emissions.**
...
**The graph shows the relationship between G7 countries SPI score and and per CO₂ emissions.**
...
**The graph shows the relationship between top emerging economies  SPI score and per capita CO₂ emissions.**
...
**The graph shows the transformation of the relationship between SPI and CO₂ emissions over time and across SPI tiers**
...
**The graph shows the change over time in CO₂ emissions for selected countries in each tier (based on 2011 SPI scores).**
...
### **Moderate improvers** : 5% to 20% decrease in CO₂ Emissions per capita in 2021
...
### **No significant change**: -5% to +5% change in CO₂ Emissions per capita 2021
...
### **Top emission increase**: 5% and more increase in CO₂ Emissions per capita 2021
...
- The best case scenario is based on assigning the lowest observed Carbon intensities to countries in each respective SPI tier (i.e, lowes carbon intensity in tier 1 to tier 1 countries and so on...).
...
widgets <- filter_select("Tier", "Select SPI Tier", sd , ~factor(tiers2011), multiple = FALSE)
...
p1<-hide_legend(ggplotly(g1,  tooltip = "text", width = 1300, height = 650))%>%
  config(displayModeBar = FALSE)
...
p2<-hide_legend(ggplotly(g2,  tooltip = "text", width = 1300, height = 650))%>%
  config(displayModeBar = FALSE)
...
p3<-hide_legend(ggplotly(g3,  tooltip = "text", width = 1300, height = 650))%>%
  config(displayModeBar = FALSE)
...
p4<-hide_legend(ggplotly(g4,  tooltip = "text", width = 1300, height = 650))%>%
  config(displayModeBar = FALSE)


tabBox(width = 12, tabPanel("SPI", p1),
                       tabPanel("Basic Human Needs", p2),
                       tabPanel("Foundations Of Wellbeign", p3),
                       tabPanel("Opportunity", p4))
```
